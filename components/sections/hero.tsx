  ...
  return (
    <section id="home" className="relative mx-auto flex min-h-screen max-w-7xl items-center px-4 pb-20 pt-32 sm:px-6 lg:px-8">
      <div className="grid w-full items-center gap-12 lg:grid-cols-[1.08fr_0.92fr]">
        <Reveal>
          <div>
            <div className="mb-6 inline-flex rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-zinc-300 backdrop-blur-xl light:border-slate-200 light:bg-white/70 light:text-slate-700">Available for modern frontend projects</div>
            <h1 className="text-balance bg-gradient-to-r from-white via-blue-100 to-accent bg-[length:200%_200%] bg-clip-text text-5xl font-semibold tracking-tight text-transparent animate-gradient light:from-slate-950 light:via-blue-700 light:to-cyan-600 sm:text-7xl lg:text-8xl">{siteConfig.name}</h1>
        ...
        <Reveal delay={0.15}>
          <div className="relative mx-auto aspect-square w-full max-w-md">
            <div className="absolute inset-0 rounded-[3rem] bg-gradient-to-br from-primary via-secondary to-accent opacity-70 blur-2xl" />
      ...
      <a href="#about" className="absolute bottom-8 left-1/2 hidden -translate-x-1/2 animate-bounce rounded-full border border-white/10 p-3 text-zinc-400 lg:block" aria-label="Scroll ke about"><ArrowDown className="h-5 w-5" /></a>
    </section>
  );
}
