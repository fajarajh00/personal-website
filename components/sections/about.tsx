        ...
        <Reveal delay={0.1}>
          <div className="grid gap-4">
            {values.map((value) => {
              ...
              return <div key={value.title} className="glass rounded-3xl p-6 transition hover:-translate-y-1"><Icon className="h-6 w-6 text-accent" /><h3 className="mt-4 font-semibold text-white light:text-slate-950">{value.title}</h3><p className="mt-2 text-sm leading-7 text-zinc-400 light:text-slate-600">{value.text}</p></div>;
            })}
          </div>
        </Reveal>
      </div>
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {experiences.map((item, index) => <Reveal key={item.role} delay={index * 0.08}><div className="h-full rounded-3xl border border-white/10 bg-white/[0.04] p-6 light:border-slate-200 light:bg-white/60"><p className="text-sm text-accent">{item.period}</p><h3 className="mt-3 font-semibold text-white light:text-slate-950">{item.role}</h3><p className="mt-1 text-sm text-zinc-500">{item.company}</p></div></Reveal>)}
      </div>
    </Section>
  );
}
